...
public class MovieController(IMovieService service) : ControllerBase
{
    ...
    public async Task<IActionResult> AddCast(
        [FromRoute] int id, [FromBody] MovieAddRemoveCastMembersDto dtoEntity)
    {
        try
        {
            await _service.AddCast(id, dtoEntity);
            return NoContent();
        }
        catch (ServiceValidationException ex)
        {
    ...
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(typeof(ErrorResponseDto), StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> RemoveCast(
        [FromRoute] int id, [FromBody] MovieAddRemoveCastMembersDto dtoEntity)
    {
        try
        {
            await _service.RemoveCast(id, dtoEntity);
            return NoContent();
        }
        catch (ServiceValidationException ex)
        {

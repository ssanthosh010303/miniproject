...
{
    private readonly IScreenService _service = service;
    ...
    [HttpPost("{id}/movieshow")]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Admin")]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ScreenAddRemoveMovieShowsDto), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ErrorResponseDto), StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> AddMovieShows([FromRoute] int id, [FromBody] ScreenAddRemoveMovieShowsDto dtoEntity)
    {
        try
        {
            return Ok(await _service.AddMovieShows(id, dtoEntity));
        }
        catch (ServiceValidationException ex)
        {
            return BadRequest(new ErrorResponseDto
    ...
    [HttpDelete("{id}/movieshow")]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Admin")]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ScreenAddRemoveMovieShowsDto), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ErrorResponseDto), StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> RemoveMovieShows([FromRoute] int id, [FromBody] ScreenAddRemoveMovieShowsDto dtoEntity)
    {
        try
        {
            return Ok(await _service.RemoveMovieShows(id, dtoEntity));
        }
        catch (ServiceValidationException ex)
        {
            return BadRequest(new ErrorResponseDto
